...
// 表示だけを担当し、認証と記録の取得はページ側で行う。
export function StudentIdView({ student, signOut }: {
    ...
    <div className="max-w-2xl space-y-8 sm:space-y-10">
      <section className="space-y-6" aria-labelledby="student-id-heading">
        ...
        <div className="panel student-number-panel space-y-5">
          <p className="student-number" aria-label={`学籍番号 ${student.studentId.split("").join(" ")}`}>
            {student.studentId}
          </p>
      ...
      <section aria-labelledby="handling-heading">
        <h2 id="handling-heading" className="text-base font-medium">取り扱いについて</h2>
